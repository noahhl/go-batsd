...
func TestFilenameCalculation(t *testing.T) {
	filename := (&Datastore{hashedFilenames: make(map[string]string, filenameHashMapSize)}).CalculateFilename("test_metric", "/u/batsd")
...
func BenchmarkFilenameCalculation(b *testing.B) {
	d := &Datastore{hashedFilenames: make(map[string]string, filenameHashMapSize)}
	for j := 0; j < b.N; j++ {
		d.CalculateFilename(metric_samples[rand.Intn(len(metric_samples))], "/u/batsd")
	}
}
...
func TestSavingToDisk(t *testing.T) {
	...
	file, err := os.Open((&Datastore{hashedFilenames: make(map[string]string, filenameHashMapSize)}).CalculateFilename("test_metric", "/tmp/batsd"))
	if err != nil {
		t.Fatalf("%v\n", err)
	}
